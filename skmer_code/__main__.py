    ...
    if (ext == 'hist'):
        ...
        for sample in ref_names:
            ...
            for i in random.randint(10,ref_hist.shape[0]-2, 5000):
                ...
                try:
                    abs_diff  = abs(math.log(ref_hist.loc[i, sample] + pseudo_count) - math.log(ref_hist.loc[i+1, sample] + pseudo_count))
                except:
                    print("error")
                    print(ref_hist.loc[i, sample] + pseudo_count)
                    print(ref_hist.loc[i+1, sample] + pseudo_count)
                    exit(1)
                if abs_diff > slope:
